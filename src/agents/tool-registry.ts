...
register({
  definition: {
    ...
    description:
      "Read what this account's Meta pixel (dataset) ACTUALLY fired, straight from Events Manager's own numbers. Use this BEFORE saying anything about an event's volume, its risk, whether it is 'low volume', or even whether it exists. Give `event` a loosely typed name (\"qualified subscriber\") and it resolves to the real event name on the pixel (\"QualifiedSubscription\") with its count, case/space/underscore-insensitive. Always returns the top events by count plus Purchase, CompleteRegistration, StartTrial and Subscribe, so a ratio (\"it fires 4.5x more often than Purchase\") is one line away. When you name an `event`, the result also carries `funnelCheck`: that event's count beside Purchase, StartTrial and CompleteRegistration, a one-word verdict (higher_than_purchase | ok | no_purchase_event), and `last7dShare` — how much of a longer window landed in its last 7 days. A downstream event CANNOT honestly out-fire the one above it, so higher_than_purchase means over-counting or a mis-named event, never healthy volume. A pixel Meta refuses to let us read comes back readable=false with the refusal — that is NOT a zero and must never be reported as 'nothing fired'.",
    input_schema: {
      type: "object" as const,
      properties: {
        clientCode: {
...
register({
  definition: {
    ...
    description:
      'THE canonical "Ready to Upload" view — use this for any "what / which ad sets are ready to upload" question (it is the same set the Launch Console and the twice-daily #ada digest use). Returns active "Upload and Configure" ad-set tasks, **already excluding Notion-Blocked** (and Done/Cancelled/Archived/Complete), each resolved to its parent ad set (ad_id_code, title, client, Notion url) and badged with pre-upload readiness (`ready` = pre-warmed + analyzed, `analyzing`, `blocked` = pre-upload flags, `stalled` = analysis terminally failed, `not-prewarmed`). Grouped by client with a `total` and per-client `count`, plus a `readiness_summary` split. PRESENTATION RULE (mandatory): only readiness=`ready` items may be presented as ready to upload; `blocked`/`stalled` items MUST be listed under their own "needs attention" heading with their flags quoted; `analyzing`/`not-prewarmed` are "in progress", not ready. Never claim "no blockers" — state each item\'s readiness badge instead. Do NOT use query_aot_tasks/query_aot_adsets and hand-filter for this question — this tool is the source of truth. Takes no parameters; returns the complete backlog (never truncated).',
    input_schema: {
      type: 'object' as const,
      properties: {},
    },
